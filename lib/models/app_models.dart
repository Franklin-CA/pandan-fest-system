...
// Replace static data here with Firebase streams when ready.
// ============================================================
  ...
  double totalWeighted(List<ActiveCriterion> criteria) {
    return scores.entries.fold(0.0, (sum, e) {
      final criterion = criteria.firstWhere(
        (c) => c.id == e.key,
        orElse: () => const ActiveCriterion(
            id: '', name: '', weight: 0, maxScore: 100),
      );
...
List<RankingEntry> computeRankings(
  ...
  judgeScores.forEach((groupId, scores) {
    final submitted =
        scores.where((j) => j.isSubmitted && j.scores.isNotEmpty).toList();
    if (submitted.isEmpty) return;

    final avg = submitted.fold(0.0, (sum, j) => sum + j.totalWeighted(criteria)) /
        submitted.length;
...
/// TODO: Replace with Firestore lookup when integrating Firebase.
...
/// Builds a resolved copy of staticJudgeScores with judge names
/// pulled from staticJudges — single source of truth.
