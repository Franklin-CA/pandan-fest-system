...
// ═══════════════════════════════════════════════════════════════════
// READY TO SCORE (NEW)
// This is shown when admin has pushed a group but judge hasn't
// clicked "Score Now" yet.
// ═══════════════════════════════════════════════════════════════════

class JudgeReadyToScore extends StatelessWidget {
  final PerformingGroup group;
  final Color categoryColor;
  final String categoryTitle;
  final IconData categoryIcon;
        ...
        child: ConstrainedBox(
          ...
          child: Column(
            ...
            children: [
              ...
              const SizedBox(height: 8),
              Text(
                'Admin has pushed this group for scoring.',
                style: GoogleFonts.poppins(
                  fontSize: 14,
                  color: const Color(0xFF8E8E93),
                ),
              ...
              Text(
                'Tapping will navigate to the scoring form\nand start the duration timer.',
                textAlign: TextAlign.center,
                style: GoogleFonts.poppins(
                  fontSize: 12,
                  color: const Color(0xFF8E8E93),
        ...
        child: ConstrainedBox(
          ...
          child: Column(
            ...
            children: [
              ...
              // ── Group cards ──
              ...groups.map((g) {
                // STRICT LOCKING: A group is locked if:
                // 1. It is not the pushed group AND it has not been scored yet.
                // 2. OR, no group is pushed at all (except for viewing already scored ones).
                //
                // Basically: if it's already scored, it's not "locked" (judge can see the "Already Scored" state).
                // If it's NOT scored, it MUST be the pushedGroupId to be selectable.
                final isScored = _isScored(g.id);
                final isPushed = pushedGroupId != null && g.id == pushedGroupId;
                
                final bool isLocked = !isScored && !isPushed;
